...
export interface IWalletTransaction extends Document {
  ...
  amount: number;
  type: 'earned' | 'spent' | 'refund' | 'admin_adjustment' | 'deposit' | 'withdrawal' | 'transfer_out' | 'transfer_in';
...
const WalletTransactionSchema: Schema<IWalletTransaction> = new Schema(
  {
    ...
    type: { 
      type: String, 
      enum: ['earned', 'spent', 'refund', 'admin_adjustment', 'deposit', 'withdrawal', 'transfer_out', 'transfer_in'], 
      required: true 
    },
    status: {
      type: String,
