...
const productSchema = z.object({
  ...
  // Validation Check: ensure at least one image is uploaded
...
export function ProductForm({ initialData }: ProductFormProps) {
  ...
  const onSubmit = async (values: ProductFormValues) => {
    ...
    const parsedStock = values.stock === '' ? 0 : Number(values.stock);
    const batches = values.batchNumber && values.batchNumber.trim() ? [{
      ...
      >
        ...
        <div className="max-w-5xl mx-auto w-full space-y-6">
          ...
          <Card>
            <CardContent className="pt-6 space-y-4">
              <div className="flex items-center justify-between">
                <Label>Attributes (Size, Color, Ingredients, etc.)</Label>
                ...
                {fields.map((item, index) => (
                  <div key={item.id} className="flex gap-4 items-end">
                    <div className="flex-1">
                      <Label>Label</Label>
                      <Input {...form.register(`attributes.${index}.key` as const)} placeholder="e.g. Volume" />
                    </div>
                    <div className="flex-1">
                      <Label>Value</Label>
                      <Input {...form.register(`attributes.${index}.value` as const)} placeholder="e.g. 200ml" />
                    </div>
                    <Button 
                      type="button"
                      variant="ghost" 
                      ...
                      <div key={field.id} className="border border-muted rounded-xl p-4 md:p-6 bg-muted/10 relative space-y-6">
                        ...
                        <div className="space-y-4 pt-4 border-t border-muted/50">
                          ...
                          <div className="space-y-3">
                            {(form.watch(`variants.${colorIndex}.sizes`) || []).map((_, sizeIndex) => (
                              <div key={sizeIndex} className="p-3 bg-background rounded-lg border border-border/60 space-y-3">
                                <div className="grid grid-cols-2 md:grid-cols-6 gap-3 items-end">
                                  <div>
                                    <Label className="text-xs font-medium text-muted-foreground">Size / Volume</Label>
                                    <Input
                                      {...form.register(`variants.${colorIndex}.sizes.${sizeIndex}.size` as const)}
                                      placeholder="e.g. 100ml / L"
                                      className="h-9 mt-1"
                                    />
                                  </div>
                                  <div>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Base physical stock available</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Lot / manufacturing batch code</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Batch expiration date</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
